...
trait Solver extends GameDef {
  ...
  def done(b: Block): Boolean = b match {
    case Block(p, _) => b.isStanding && (p == goal)
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filter {case (b, _) =>
    !explored.contains(b)}
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter {case (b, hs) => done(b)}
   ...
   * position.
   */
  lazy val solution: List[Move] = if(pathsToGoal.isEmpty) List() else pathsToGoal.head match {case (b, hs) => hs}
}
